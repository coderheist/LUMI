...
const CHAPTERS = [
  ...
  },
  {
    ...
    items: [
      ...
      "`--color-sage` / `--color-ochre` — resolved/automated and escalated/pending states",
...
/** All client-side: seven chapters doesn't warrant an index, just a filter
 *  over the text already on the page. */
export function DocsContent() {
  ...
  const filtered = useMemo(() => {
    ...
    return CHAPTERS.filter((chapter) => {
      const haystack = [chapter.heading, ...chapter.body, ...("items" in chapter ? chapter.items : [])]
        .join(" ")
        .toLowerCase();
    ...
    <Section space="normal">
      <div className="shell grid gap-12 lg:grid-cols-12 lg:gap-16">
          ...
          <div className="lg:sticky lg:top-28">
            <label className="relative block">
              ...
              <input
                ...
                className="h-10 w-full rounded-chip border border-line-strong bg-paper pl-9 pr-3 text-[0.84rem] transition-colors duration-200 hover:border-ink focus:border-ink"
              />
            </label>

            <p className="t-micro mb-3 mt-6">Contents</p>
        ...
        <div className="flex flex-col gap-14 lg:col-span-9">
          {filtered.length === 0 ? (
            <p className="t-body">No sections match &ldquo;{query}&rdquo;.</p>
          ) : (

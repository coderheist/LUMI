  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h3 className="text-[1rem] font-medium text-paper">Conversations by week</h3>
          ...
          {RANGES.map((range) => (
            <button
              ...
              className={`rounded-[7px] px-2.5 py-1.5 text-[0.78rem] font-medium transition-colors duration-200 ${
                weeks === range.weeks ? "bg-paper text-ink" : "text-paper/55 hover:text-paper"
              }`}
            >
              {range.label}
            </button>
      ...
      <div className="mt-7 flex gap-3">
        ...
        <div className="flex h-[15rem] w-9 shrink-0 flex-col justify-between">
          {ticks.map((tick) => (
            <span key={tick} data-numeric className="text-[0.68rem] leading-none text-paper/35">
              {tick}
            </span>
          ))}
        </div>
        ...
        <div className="relative min-w-0 flex-1">
          ...
          <div aria-hidden className="absolute inset-0 flex flex-col justify-between">
            {ticks.map((tick) => (
              <span key={tick} className="h-px w-full bg-white/[0.07]" />
            ))}
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      height: `${(row.escalated / axisMax) * 100}%`,
                  ...
                  <div
                    ...
                    style={{
                      height: `${(row.automated / axisMax) * 100}%`,
                  ...
                  {isHovered ? (
                    <div className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 w-[11rem] -translate-x-1/2 rounded-card border border-indigo-line bg-indigo-ink p-3 shadow-float">
                      <p data-numeric className="text-[0.76rem] font-medium text-paper">
                        {row.week}
                      </p>
                      <dl className="mt-2 flex flex-col gap-1.5">
                        {(["automated", "escalated"] as const).map((key) => (
                          <div key={key} className="flex items-center gap-2">
                            ...
                            />
                            <dt className="flex-1 text-[0.72rem] text-paper/55">
                              {SERIES[key].label}
                            </dt>
                            <dd data-numeric className="text-[0.72rem] text-paper">
                              {row[key]}
                            </dd>
                          </div>
                        ))}
                        <div className="mt-0.5 flex items-center gap-2 border-t border-indigo-line pt-1.5">
                          <dt className="flex-1 text-[0.72rem] text-paper/55">Total</dt>
                          <dd data-numeric className="text-[0.72rem] text-paper">
                            {total}
                          </dd>
                        </div>
          ...
          <div className="mt-2.5 flex gap-[3px] sm:gap-1.5">
            {rows.map((row) => (
              <span
                key={row.week}
                data-numeric
                className="flex-1 text-center text-[0.66rem] text-paper/35"
              >
                {row.week}
              </span>
            ))}
        ...
        {(["automated", "escalated"] as const).map((key) => (
          <span key={key} className="inline-flex items-center gap-2">
            <span
              ...
              style={{ backgroundColor: SERIES[key].colour }}
            />
            <span className="text-[0.8rem] text-paper/65">{SERIES[key].label}</span>
          </span>
        ))}
      </div>

      <details className="mt-5">
        <summary className="cursor-pointer text-[0.78rem] text-paper/45 transition-colors duration-200 hover:text-paper/75">
          View as table
        </summary>
          ...
          <thead>
            <tr className="border-b border-indigo-line">
              {["Week", "Resolved by Lumi", "Escalated", "Total"].map((heading) => (
                <th key={heading} className="py-2 text-[0.74rem] font-medium text-paper/55">
                  {heading}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.week} className="border-b border-indigo-line/60">
                <td data-numeric className="py-2 text-[0.76rem] text-paper/75">{row.week}</td>
                <td data-numeric className="py-2 text-[0.76rem] text-paper/75">{row.automated}</td>
                <td data-numeric className="py-2 text-[0.76rem] text-paper/75">{row.escalated}</td>
                <td data-numeric className="py-2 text-[0.76rem] text-paper">
